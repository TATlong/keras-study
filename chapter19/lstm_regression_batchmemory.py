...
"""
    ...
    在keras中通过设置LSTM层的stateful为True来保存LSTM层的内部状态，从而获得更好的控制。这样就可以在
    整个序列上构建状态，甚至在需要进行预测时也保持该状态。
"""
...
def build_model():
    ...
    model.add(LSTM(units=4,
                   batch_input_shape=(batch_size, look_back, 1),  # 每个epoch中的样本数量进行硬编码
                   stateful=True))
...
if __name__ == '__main__':
    ...
    # 必须创建外部循环的epoch：并在每个epoch中调用model.fit和model.reset_states
