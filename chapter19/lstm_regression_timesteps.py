...
"""
    在一些时间序列问题中，每个样本都有不同的时间步长，例如：对一个机器故障进行测量的数据集中，每一个故障点
    都是一个样本， 观察到的结果是时间步长，观察到的变量是特征。
    
    这时：时间步长是输入+过去观察变量，当前变量是输出
    而不是将过去的观察变量作为单独的输入特征，这种将时间步长作为输入特征的方法能提高时间序列问题的准确度。
